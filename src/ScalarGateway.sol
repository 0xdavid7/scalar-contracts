...
contract ScalarGateway is IAxelarGateway, Implementation, EternalStorage {
  ...
  function setup(bytes calldata params) external override(IImplementation, Implementation) onlyProxy {
    (address governance_, address mintLimiter_, bytes memory newOperatorsData) = abi.decode(
      params,
  ...
  function execute(bytes calldata input) external override {
    ...
    for (uint256 i; i < commandsLength; ++i) {
      ...
      if (commandHash == SELECTOR_DEPLOY_TOKEN) {
        commandSelector = ScalarGateway.deployToken.selector;
      } else if (commandHash == SELECTOR_MINT_TOKEN) {
        commandSelector = ScalarGateway.mintToken.selector;
      } else if (commandHash == SELECTOR_APPROVE_CONTRACT_CALL) {
        commandSelector = ScalarGateway.approveContractCall.selector;
      } else if (commandHash == SELECTOR_APPROVE_CONTRACT_CALL_WITH_MINT) {
        commandSelector = ScalarGateway.approveContractCallWithMint.selector;
      } else if (commandHash == SELECTOR_BURN_TOKEN) {
        commandSelector = ScalarGateway.burnToken.selector;
      } else if (commandHash == SELECTOR_TRANSFER_OPERATORSHIP) {
        if (!allowOperatorshipTransfer) continue;

        allowOperatorshipTransfer = false;
        commandSelector = ScalarGateway.transferOperatorship.selector;
  ...
  function _getIsContractCallApprovedKey(
    ...
    return
      keccak256(
        abi.encode(PREFIX_CONTRACT_CALL_APPROVED, commandId, sourceChain, sourceAddress, contractAddress, payloadHash)
      );
  }
